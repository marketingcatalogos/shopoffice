document.addEventListener('DOMContentLoaded', function () {
  ...
  /* ---------- Modais de galeria dos produtos (<dialog> nativo) ----------
     Delegado no document: os produtos (e seus modais) são criados
     dinamicamente pelo catalog.js depois que o CSV carrega, então não dá
     pra prender o listener em elementos que ainda não existem. */
  document.addEventListener('click', function (e) {
    ...
    if (closeBtn) {
      var openDialog = closeBtn.closest('dialog');
      if (openDialog) openDialog.close();
      return;
    }

    // Fecha ao clicar fora da caixa de conteúdo (no backdrop do próprio <dialog>)
  ...
  chatMessages.addEventListener('click', function (e) {
    ...
    if (whatsappBtn) {
      var link = typeof window.getShopofficeQuoteWhatsAppLink === 'function'
        ...
        : 'https://wa.me/557797730178880?text=' + encodeURIComponent('Quero fazer um orçamento.');
...
/* =========================================================================
   ...
   ========================================================================= */
(function () {
  // Número de WhatsApp da Shopoffice, formato 55 + DDD + número (só dígitos).
  var WHATSAPP_NUMBER = '557730178880';
  ...
  /* ---------- Busca os dados de um produto na lista carregada pelo catalog.js ---------- */
  function getProduct(id) {
    var lista = window.SHOPOFFICE_PRODUCTS || [];
    for (var i = 0; i < lista.length; i++) {
      if (lista[i].id === id) {
        return { title: lista[i].nome, image: lista[i].imagem_capa, category: lista[i].categoria };
      }
    }
    return null;
  }
  ...
  function setQuantity(id, qty) {
    var produto = getProduct(id);
    if (!produto) return;
    qty = Math.max(0, Math.min(99, qty));
    if (qty === 0) {
      delete cart[id];
    } else {
      cart[id] = { id: id, title: produto.title, image: produto.image, category: produto.category, qty: qty };
    }
    updateAll();
  }
  ...
  /* ---------- Steppers nos cards + botão "Adicionar ao Orçamento" nos modais ----------
     Delegado no document pelo mesmo motivo dos modais: os cards e modais de
     produto são criados dinamicamente pelo catalog.js, não existem ainda
     no momento em que este script roda. */
  document.addEventListener('click', function (e) {
    var qtyBtn = e.target.closest('[data-qty-action]');
    if (qtyBtn) {
      var stepper = qtyBtn.closest('.qty-stepper');
      if (stepper) {
        var stepperId = stepper.getAttribute('data-product-id');
        changeQuantity(stepperId, qtyBtn.getAttribute('data-qty-action') === 'increase' ? 1 : -1);
      }
      return;
    }
    ...
    if (addBtn) {
      ...
      changeQuantity(addId, 1);
      var dialog = addBtn.closest('dialog');
